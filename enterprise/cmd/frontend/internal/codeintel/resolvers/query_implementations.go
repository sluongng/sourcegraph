...
func (r *queryResolver) Implementations(ctx context.Context, line, character int, limit int, rawCursor string) (_ []AdjustedLocation, _ string, err error) {
	...
	if cursor.Phase == "local" {
		for len(locations) < limit {
			localLocations, hasMore, err := r.pageLocalReferences(ctx, "implementations", adjustedUploads, &cursor.LocalCursor, limit-len(locations))
			if err != nil {
				return nil, "", err
			}
	...
	if cursor.Phase == "dependents" {
		for len(locations) < limit {
			remoteLocations, hasMore, err := r.pageRemoteReferences(ctx, "implementations", adjustedUploads, cursor.OrderedExportMonikers, &cursor.RemoteCursor, limit-len(locations))
			if err != nil {
				return nil, "", err
			}
