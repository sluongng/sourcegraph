...
func (r *searchResolver) doResults(ctx context.Context, args *search.TextParameters, jobs []run.Job) (res *SearchResults, err error) {
	...
	skipUnindexed := args.Mode == search.SkipUnindexed || (args.Mode == search.ZoektGlobalSearch && envvar.SourcegraphDotComMode())
	...
	if args.Mode == search.ZoektGlobalSearch {
		...
		if args.ResultTypes.Has(result.TypeFile | result.TypePath) {
			...
			goroutine.Go(func() {
				defer wg.Done()
				ctx, stream, cleanup := streaming.WithLimit(ctx, stream, int(argsIndexed.PatternInfo.FileMatchLimit))
				defer cleanup()
				...
				_ = agg.DoFilePathSearch(ctx, zoektArgs, searcherArgs, true)
			})
		}
